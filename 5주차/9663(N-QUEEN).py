...
def n_queen(n):
    count = 0                        # 퀸을 올바르게 놓을 수 있는 경우의 수를 세기 위해 count라는 변수를 0부터 시작
    board = [-1] * n                 # board = [-1] * n은 처음에는 모든 행에 퀸이 없기 때문에 -1로 초기화를 해둠
                                     # 아직 행에 퀸을 두기 전의 상태이기 때문이다.

    # 현재 행(row)에 열(col)에 퀸을 놓을 수 있는지 검사
    def is_safe(row, col):                                  # row : 행, col : 열
        for prev in range(row):                             # 퀸을 행마다 한줄 씩 놓아 가면서 이전에 놓은 퀸들과 만나는 지점에서 충돌하는지 검사 해야 하기 때문에
                                                            # range(row)는 이전의 행들 (0부터 row-1번째까지 범위 모두 순회), prev : 이전에 퀸을 배치한 행 번호.
    ...
    backtrack(0)
    return count # 0 번째 행부터 시작하고, 모든 경우를 다 시도해 본 후 가능한 배치 수를 반환한다.
